...
import yanzj
from yanzj.navigation.utils import OrbSlamVisualizer
...
def base_keyboard_teleop(apply_translation_vel=0.12, apply_rotational_vel=0.7):
    ...
    while not keys["escape"]:
        ...
        if translation_vel == 0.0 and rotational_vel == 0.0:
            continue
        yanzj.set_base_velocity(translation_vel, rotational_vel)
...
def feature_matching_dance():
    ...
    for d in deltas:
        yanzj.move_by(joint_head_pan=d, joint_head_tilt=d)
        time.sleep(0.05)
...
if __name__ == "__main__":
    ...
    visualizer = OrbSlamVisualizer(
        args.vocab_path, args.config_path, yanzj._client.ip_addr, yanzj._client.port
    )
