...
path = os.path.expanduser("~/.yanzj")


def _ensure_path_exists() -> None:
    """Ensure that the ~/.yanzj directory exists."""
...
def lookup_address(
    ...
    """Return the address of the robot. Will also create and update ~/.yanzj/robot_ip.txt file to manage robot IP address.
    ...
    if use_remote_computer:
        if len(robot_ip) > 0:
            ...
            if update:
                with open(os.path.expanduser("~/.yanzj/robot_ip.txt"), "w") as f:
                    f.write(robot_ip)
        else:
            # Look up the robot computer in config directory
            if not os.path.exists(os.path.expanduser("~/.yanzj/robot_ip.txt")):
                return None
            robot_ip = open(os.path.expanduser("~/.yanzj/robot_ip.txt")).read().strip()
...
def get_path_to_map(name: str) -> str:
    """Gets a map filename in the .yanzj store directory"""
    _ensure_path_exists()
...
def get_path_to_image(name: str) -> str:
    """Gets an image filename in the .yanzj store directory"""
    _ensure_path_exists()
...
def get_path_to_debug(name: str) -> str:
    """Gets a debug filename in the .yanzj store directory"""
    _ensure_path_exists()
