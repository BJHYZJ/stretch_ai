...
class Detic:
    def __init__(self, config: DictConfig, visualize: bool = False) -> None:
        if config.vocabulary == "custom":
            ...
            self.metadata = MetadataCatalog.get(BUILDIN_METADATA_PATH[config.vocabulary])
    ...
    def get_goal_mask(self, goal_img: np.ndarray) -> Tuple[np.ndarray, Optional[VisImage]]:
        """
