...
from yanzj.perception.constants import RearrangeDETICCategories
from yanzj.utils.config import get_full_config_path
...
class OvmmPerception:
    ...
    def __init__(
        ...
        if self._detection_module == "detic":
            # Lazy import
            from yanzj.perception.detection.detic import DeticPerception
            ...
            from yanzj.perception.detection.sam import SAMPerception

            self._segmentation = SAMPerception()

        elif self._detection_module == "mobile_sam":
            from yanzj.perception.detection.mobile_sam import MobileSAMPerception

            self._segmentation = MobileSAMPerception()
        elif self._detection_module == "sam2":
            from yanzj.perception.detection.sam2 import SAM2Perception

            self._segmentation = SAM2Perception()

        elif self._detection_module == "yolo":
            from yanzj.perception.detection.yolo import YoloPerception
            ...
            from yanzj.perception.detection.yolo_world import YoloWorldPerception
